...
def dataset_process(i, out_dir, config):
    ...
    for data in tqdm(iter(dataset)):
        ...
        data_new = {
            "vehicles_current_occupancy": tf.squeeze(timestep_grids.vehicles.current_occupancy, axis=0).numpy(),
            "vehicles_past_occupancy": tf.squeeze(timestep_grids.vehicles.past_occupancy, axis=0).numpy(),
            "pedestrians_current_occupancy": tf.squeeze(timestep_grids.pedestrians.current_occupancy,
                                                        axis=0).numpy(),
            "pedestrians_past_occupancy": tf.squeeze(timestep_grids.pedestrians.past_occupancy, axis=0).numpy(),
            "cyclists_current_occupancy": tf.squeeze(timestep_grids.cyclists.current_occupancy, axis=0).numpy(),
            "cyclists_past_occupancy": tf.squeeze(timestep_grids.cyclists.past_occupancy, axis=0).numpy(),
...
def data_process():
    ...
    # Replace this path with your own tfrecords.
    # DATASET_FOLDER = '/path/to/waymo_open_dataset_motion_v_1_1_0/uncompressed'
    DATASET_FOLDER ='/home/weihaoran/waymo_data'
    ...
    out_dir = '/home/weihaoran/waymo_data/train_processed_database_origin'
