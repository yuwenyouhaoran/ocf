...
def render_occupancy_from_inputs(
  ...
  hight = _stack_field(inputs, times, 'width')

  length = tf.broadcast_to(length, agent_type.shape)
  width = tf.broadcast_to(width, agent_type.shape)
  hight = tf.broadcast_to(hight, agent_type.shape)
  # dynamic x,y,vx,vy
  velocity_y = _stack_field(inputs, times, 'velocity_y')
  velocity_x = _stack_field(inputs, times, 'velocity_x')
  vel_yaw = _stack_field(inputs, times, 'vel_yaw')
  speed = tf.cos(velocity_x) + tf.sin(velocity_y)
  velocity_x = tf.broadcast_to(velocity_x, agent_type.shape)
  velocity_y = tf.broadcast_to(velocity_y, agent_type.shape)
  speed = tf.broadcast_to(speed, agent_type.shape)
  ...
  for object_type in occupancy_flow_data.ALL_AGENT_TYPES:
    ...
    if object_type == 1:
        ...
        velocity_x_img_coord = tf.gather_nd(velocity_x, point_indices)[..., tf.newaxis]
        velocity_y_img_coord = tf.gather_nd(velocity_y, point_indices)[..., tf.newaxis]
        speed_img_coord = tf.gather_nd(speed, point_indices)[..., tf.newaxis]

        # print(x_img_coord)
        # print('velocity_x_img_coord',velocity_x_img_coord)
        num_points_to_render = point_indices.shape.as_list()[0]
        # print('num_points_to_render',num_points_to_render)
        xy_img_coord = tf.concat(
            ...
            axis=1, )
        #         print('xy_img_coord',xy_img_coord.shape)
        gt_values = tf.squeeze(tf.ones_like(x_img_coord, dtype=tf.float32), axis=-1)
        #         print('gt_values',gt_values.shape)
        topdown = tf.scatter_nd(xy_img_coord, gt_values, topdown_shape)
        topdown = tf.clip_by_value(topdown, 0.0, 1.0)
        #         print('x_img_coord',x_img_coord)
        vx_inf = tf.squeeze(velocity_x_img_coord, axis=-1)
        vy_inf = tf.squeeze(velocity_y_img_coord, axis=-1)
        speed_inf = tf.squeeze(speed_img_coord, axis=-1)
        #         print(topdown)
        vx_topdown = tf.scatter_nd(xy_img_coord, vx_inf, topdown_shape)
        vy_topdown = tf.scatter_nd(xy_img_coord, vy_inf, topdown_shape)
        speed_topdown = tf.scatter_nd(xy_img_coord, speed_inf, topdown_shape)
        #         topdown=tf.concat([topdown,vx_topdown,vy_topdown,speed_topdown],axis=-1)
        if times == ['current']:
            length_img_coord = tf.gather_nd(length, point_indices)[..., tf.newaxis]
            width_img_coord = tf.gather_nd(width, point_indices)[..., tf.newaxis]
            hight_img_coord = tf.gather_nd(hight, point_indices)[..., tf.newaxis]
            length_inf = tf.squeeze(length_img_coord, axis=-1)
            width_inf = tf.squeeze(width_img_coord, axis=-1)
            hight_inf = tf.squeeze(hight_img_coord, axis=-1)
            length_topdown = tf.scatter_nd(xy_img_coord, length_inf, topdown_shape)
            width_topdown = tf.scatter_nd(xy_img_coord, width_inf, topdown_shape)
            hight_topdown = tf.scatter_nd(xy_img_coord, hight_inf, topdown_shape)
            static_topdown = tf.concat([length_topdown, width_topdown, hight_topdown], axis=-1)
            #             print('static_topdown',static_topdown.shape)
            topdown = tf.concat([topdown, vx_topdown, vy_topdown, speed_topdown, static_topdown], axis=-1)
        else:
            topdown = tf.concat([topdown, vx_topdown, vy_topdown, speed_topdown], axis=-1)
...
def render_trafficgraph_from_inputs(
    inputs: Mapping[str, tf.Tensor],
    config: occupancy_flow_metrics_pb2.OccupancyFlowTaskConfig,
) -> tf.Tensor:
    batch_size, _, _ = (
        inputs['roadgraph_samples/xyz'].shape.as_list())
    topdown_shape = [batch_size, config.grid_height_cells, config.grid_width_cells, 1]

    sdc_x = inputs['sdc/current/x'][:, tf.newaxis, :]
    sdc_y = inputs['sdc/current/y'][:, tf.newaxis, :]
    sdc_z = inputs['sdc/current/z'][:, tf.newaxis, :]

    traffic_light_x = (inputs['traffic_light_state/current/x'] - sdc_x)[:, 0, :]
    traffic_light_y = (inputs['traffic_light_state/current/y'] - sdc_y)[:, 0, :]
    traffic_light_z = (inputs['traffic_light_state/current/z'] - sdc_z)[:, 0, :]
    traffic_light_valid = inputs['traffic_light_state/current/valid']
    traffic_light_state = inputs['traffic_light_state/current/state'][:, 0, :]
    traffic_light_states = []
    for i in range(0, 9):
        if i == 0:
            traffic_light_states.append(tf.equal(traffic_light_state, i - 1))
        else:
            traffic_light_states.append(tf.equal(traffic_light_state, i))
    #     print('traffic_light_states{}:{}'.format(i,traffic_light_states[i]))

    # print('traffic_light_x',traffic_light_x)
    # print('traffic_light_y',traffic_light_y)
    # print('traffic_light_valid',traffic_light_valid.shape)
    # print('traffic_light_state',traffic_light_state.shape)
    # print('traffic_light_state',traffic_light_state)
    if config.normalize_sdc_yaw:
        angle = math.pi / 2 - inputs['sdc/current/bbox_yaw']
        traffic_light_x, traffic_light_y = rotate_points_around_origin(traffic_light_x, traffic_light_y, angle)
    traffic_light_x, traffic_light_y, traffic_light_is_in_fov = _transform_to_image_coordinates(
        points_x=traffic_light_x,
        points_y=traffic_light_y,
        config=config,
    )
    traffic_light_is_valid = tf.cast(traffic_light_valid[:, 0, :], tf.bool)
    should_render_traffic_light = tf.logical_and(traffic_light_is_in_fov, traffic_light_is_valid)
    traffic_light_viz_states = []
    for i in range(0, 9):
        should_render_traffic_light_state = tf.logical_and(should_render_traffic_light, traffic_light_states[i])
        traffic_light_indices = tf.cast(tf.where(should_render_traffic_light_state), tf.int32)
        x_img_coord_type = tf.gather_nd(traffic_light_x, traffic_light_indices)[..., tf.newaxis]
        y_img_coord_type = tf.gather_nd(traffic_light_y, traffic_light_indices)[..., tf.newaxis]
        num_traffic_light_to_render_state = traffic_light_indices.shape.as_list()[0]
        xy_img_coord = tf.concat(
            [
                traffic_light_indices[:, :1],
                tf.cast(y_img_coord_type, tf.int32),
                tf.cast(x_img_coord_type, tf.int32),
            ],
            axis=1,
        )
        #     print('type_point_indices',type_point_indices[i][:, :1])
        #     print('xy_img_coord_type',xy_img_coord)
        #     print('x_img_coord_type',x_img_coord_type[i].shape)
        gt_values = tf.ones_like(x_img_coord_type, dtype=tf.float32)
        #     print("gt_value_{}:{}".format(i,gt_values.shape))
        #     print('gt_values',gt_values)
        #     print('xy_img_coord',xy_img_coord)
        traffic_light_viz = tf.scatter_nd(xy_img_coord, gt_values, topdown_shape)
        #     print('rg_viz_{}:{}'.format(i,rg_viz.shape))
        #     print('rg_viz',rg_viz)
        traffic_light_viz = tf.clip_by_value(traffic_light_viz, 0.0, 1.0)
        traffic_light_viz_states.append(traffic_light_viz)
    #     print('traffic_light_viz_states{}:{}'.format(i,traffic_light_viz_states[i].shape))
    traffic_light_viz = tf.concat(traffic_light_viz_states, axis=-1)
    return traffic_light_viz
def render_roadgraph_from_inputs(
  ...
  rg_type = inputs['roadgraph_samples/type'][..., 0]
  rg_points_type = []
  for i in range(0, 20):
      if i == 0:
          rg_points_type.append(tf.equal(rg_type, i - 1))
      else:
          rg_points_type.append(tf.equal(rg_type, i))
  ...
  rg_viz_type = []
  for i in range(0, 20):
      should_render_point_type = tf.logical_and(should_render_point, rg_points_type[i])
      type_point_indices = tf.cast(tf.where(should_render_point_type), tf.int32)
      x_img_coord_type = tf.gather_nd(rg_x, type_point_indices)[..., tf.newaxis]
      y_img_coord_type = tf.gather_nd(rg_y, type_point_indices)[..., tf.newaxis]
      num_points_to_render_type = type_point_indices.shape.as_list()[0]
      xy_img_coord = tf.concat(
          [
              type_point_indices[:, :1],
              tf.cast(y_img_coord_type, tf.int32),
              tf.cast(x_img_coord_type, tf.int32),
          ],
          axis=1,
      ...
      rg_viz_type.append(rg_viz)
  rg_viz = tf.concat(rg_viz_type, axis=-1)
...
def _sample_and_filter_agent_points(
  ...
  points_per_agent = (
      config.agent_points_per_side_length * config.agent_points_per_side_width) #768
  ...
  field_shape = [batch_size, num_agents, num_steps, points_per_agent]  #[batch,128,1/10/80,768]
  assert_shapes([
...
def _stack_field(
  ...
  if field == 'type':
    ...
    fields = inputs['state/type']  #(batch,128)
