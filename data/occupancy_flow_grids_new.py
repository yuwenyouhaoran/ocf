...
import occupancy_flow_renderer_new
...
class TrafficGrids:
  # Roadgraph elements.
  # [batch_size, height, width, 1]
  trafficgraph: Optional[tf.Tensor] = None
  # Trail of scene agents over past and current time frames.
  # [batch_size, height, width, 1]

def create_ground_truth_traffic_grids(
   inputs: Mapping[str, tf.Tensor],
    config: occupancy_flow_metrics_pb2.OccupancyFlowTaskConfig,
) -> TrafficGrids:
    roadgraph = occupancy_flow_renderer_new.render_trafficgraph_from_inputs(
        inputs, config)
...
def create_ground_truth_timestep_grids(
  ...
  render_func = functools.partial(
      occupancy_flow_renderer_new.render_occupancy_from_inputs,
      inputs=inputs,
      config=config)
...
def create_ground_truth_vis_grids(
  ...
  """
  roadgraph = occupancy_flow_renderer_new.render_roadgraph_from_inputs(
      inputs, config)
